...
run_directory <- "3394_T1-T3_RNA-TCR"
rna_directories <- c('T1_B','T2_A','T3') # List the RNAseq directory names you want to analyze
tcr_directories <- c('T1_B-TCR','T2_A-TCR','T3-TCR') # List the TCR directory names you want to analyze in order that corresponds to 'cite_directories'
...
for (i in 1:length(rna_directories)) {
  ...
  # Load GEX and HASH
  rna_dir = paste('../../Raw_Sequencing', run_directory, rna_directories[i], "outs", "filtered_feature_bc_matrix.h5", sep='/')
  
  data_read = Read10X_h5(rna_dir, use.names = TRUE, unique.features = TRUE) # Read data into Seurat
  
  if (rna_directories[i] == "T3") {
    seurat_object = CreateSeuratObject(counts = data_read)
  } else {
  ...
  # Load TCR
  tcr_dir = paste('../../Raw_Sequencing', run_directory, tcr_directories[i], "outs", 'filtered_contig_annotations.csv', sep='/')
...
for (i in 1:length(cohortid_timepoints)) {
  ...
  sample.data@meta.data$stim <- 'iRBCs'
...
# Min/max features
```{r}
min_features <- c(500, 500, 500)
max_features <- c(4000, 4000, 3500)
