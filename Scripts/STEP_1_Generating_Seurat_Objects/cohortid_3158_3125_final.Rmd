...
rna_directories <- c('S1','S2') # List the RNAseq directory names you want to analyze
tcr_directories <- c('S1','S2') # List the TCR directory names you want to analyze in order that corresponds to 'cite_directories'
cohortids <- c('3158','3125')
timepoint <- "T1"
...
```{r}
unstimulated_cutoffs <- c(0.7, 1.2)
iRBCs_cutoffs <-        c(1.0, 0.4)
mns_cutoffs <-          c(1.0, 0.65)
...
```{r}
## Merge
data.combined <- merge(individual_seurat_objects[[1]], y = individual_seurat_objects[-1], add.cell.ids = cohortid_timepoints, project = "3158_3125")
...
```{r}
saveRDS(data.combined, file = paste("../../Seurat_Objects/rna_tcr_3158_3125.rds", sep=""))
```
