...
run_directory <- "3149_3178_RNA-TCR"
rna_directories <- c('S5','S6','S7','S8') # List the RNAseq directory names you want to analyze
tcr_directories <- c('S5','S6','S7','S8') # List the TCR directory names you want to analyze in order that corresponds to 'cite_directories'
...
for (i in 1:length(rna_directories)) {
  ...
  # Label sample
  seurat_object@meta.data$sample <- paste(cohortid, rna_directories[i], sep="_")
  ...
  combined_tcr <- combineTCR(annotations, samples=rna_directories[i], ID=rna_directories[i], 
                             cells ="T-AB", filterMulti = FALSE)[[1]]
  ...
  cohortid_timepoint <- paste(cohortid, rna_directories[i], sep="_")
...
```{r}
unstimulated_cutoffs <- c(0.7, 1.0, 1.00, 1)
iRBCs_cutoffs <-        c(1.0, 0.5, 0.75, 0.75)
bead_cutoffs <-         c(1.0, 9.0, 0.90, 1.3)
...
for (i in 1:length(cohortid_timepoints)) {
  ...
  colnames(data_for_plot) <- c('unstimulated','iRBCs', 'bead_stimulated')
  ...
  q <- ggplot(data_for_plot, aes(x=unstimulated, y=bead_stimulated)) + 
    geom_point(alpha=0.1) +
    
    geom_hline(yintercept=bead_cutoffs[i], linetype="dashed", color = "red") + 
    geom_vline(xintercept=unstimulated_cutoffs[i], linetype="dashed", color = "red")
...
for (i in 1:length(cohortid_timepoints)) {
  ...
  colnames(data_for_subsetting) <- c('unstimulated','iRBCs','bead_stimulated')
  ...
  barcodes_iRBCs <- row.names(subset(data_for_subsetting, 
                                     ...
                                     data_for_subsetting$bead_stimulated < bead_cutoffs[i])
                              )
  ...
  barcodes_beads <- row.names(subset(data_for_subsetting, 
                                     data_for_subsetting$iRBCs < iRBCs_cutoffs[i] & 
                                     data_for_subsetting$unstimulated < unstimulated_cutoffs[i] &
                                     data_for_subsetting$bead_stimulated > bead_cutoffs[i])
                              )
  ...
  # If statement because T2 does not have any bead stimulated cells
  if (i != 2) {
    sample.data@meta.data[row.names(sample.data@meta.data) %in% barcodes_beads,]$stim <- "bead_stimulated"
  }
...
# Min/max features
```{r}
min_features <- c(500, 500, 500, 500)
max_features <- c(5000, 5000, 5000, 5000)
